...
class Stage extends FlxTypedGroup<FlxBasic>
{
	...
	public function new(curStage)
	{
		...
		if (PlayState.determinedChartType == "FNF")
		{
			...
			{
				case 'flutter' | 'roughScales' | 'ophiogomphus':
					curStage = 'swamp';
		...
		switch (curStage)
		{
			...
			case 'swamp':
				...
				var bg:FlxSprite = new FlxSprite(-520, -120).loadGraphic(Paths.image('backgrounds/' + curStage + '/stage'));
				bg.antialiasing = true;
				bg.scrollFactor.set(0.9, 0.9);
				bg.active = false;
				add(bg);
				
				var fg:FlxSprite = new FlxSprite(-500, 360).loadGraphic(Paths.image('backgrounds/' + curStage + '/foreground'));
				fg.antialiasing = true;
				fg.scrollFactor.set(0.9, 0.9);
				fg.setGraphicSize(Std.int(fg.width * 0.95));
				fg.active = false;
				add(fg);
